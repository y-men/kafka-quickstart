...
//   // @StreamListener
//    public void readDriversInfo(
//            @Input("out")
//                    KTable<String, Long> out) {
...
//
    ...
    void driversOnShiftWithTasksAssigned(@Input("tasks") KStream<Object, Task> tasks,
        ...
        shifts
                ...
                .count(
//                         //Materialized.as("drivers-with-tasks-on-shift").with( Serdes.String(),Serdes.Long())
                         Materialized.<String, Long, KeyValueStore<Bytes, byte[]>>as("qstore")
                                 .withKeySerde(Serdes.String())
                                 .withValueSerde(Serdes.Long())
                )
         //       .toStream().to("out1");
//           .toStream().to("qstore" ,Produced.with( Serdes.String(),Serdes.Long()));
        ;
